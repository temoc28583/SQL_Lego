...
--   parent_id matches an id in the previous batch, attaching each child
--   onto its parent. Repeats layer by layer until a round finds no
...
--   producing "Technic > Star Wars" — a theme can sit under an unexpected
--   parent, so ancestry isn't always what the name suggests.